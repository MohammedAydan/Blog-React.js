...
function ManageRequestsUserProfile({
  ...
  const handleAcceptRequest = async () => {
    setRequestLoading(true);
    try {
      const resUser = await axiosClient.put(`/friends/${request.request.id}`);
      if (resUser.status == 200) {
        setRequest(resUser.data);
      }
    } catch (error) {
  ...
  return (
    <div className="flex items-center justify-center mt-7 border-t-2 border-t-slate-800 p-4">
      ...
      <IFCondition condition={!requestLoading && request == null}>
        <PrimaryButton onClick={handleSendRequest}>Request</PrimaryButton>
      </IFCondition>
      ...
      <IFCondition
        condition={
          !requestLoading && request != null && request.request.status == true
        }
      >
        <DangerButton onClick={handleCancelRequest}>Cancel friend</DangerButton>
      </IFCondition>
    </div>
  );
}
